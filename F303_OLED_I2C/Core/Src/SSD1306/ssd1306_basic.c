...
/**
 * @brief  Removes the cursor
 * @param  ssd1306: ssd1306_t selected
 */
void SSD1306_ResetCursor(ssd1306_t* ssd1306) {
	uint16_t x_position = 2;
	uint16_t y_position = 0;

	SSD1306_GotoXY(ssd1306, x_position, y_position);
	SSD1306_Putc(ssd1306, ' ', &Font_11x18, 1);
	SSD1306_GotoXY(ssd1306, x_position, y_position+Font_11x18.FontHeight);
	SSD1306_Putc(ssd1306, ' ', &Font_11x18, 1);
	SSD1306_GotoXY(ssd1306, x_position, y_position+2*Font_11x18.FontHeight);
	SSD1306_Putc(ssd1306, ' ', &Font_11x18, 1);
	SSD1306_GotoXY(ssd1306, x_position, y_position+3*Font_11x18.FontHeight-1);
	SSD1306_Putc(ssd1306, ' ', &Font_7x10, 1);
	SSD1306_GotoXY(ssd1306, SSD1306_WIDTH/2, y_position+3*Font_11x18.FontHeight-1);
	SSD1306_Putc(ssd1306, ' ', &Font_7x10, 1);
}

/**
 * @brief  Set cursor on a specific position
 * @note   @ref Only 5 slots are accepted (3 large and 2 small)
 * @param  ssd1306: ssd1306_t selected
 * @param  position: The position at which the cursor will be set: 0, 1, and 2 are large. 3 and 4 are small
 */
void SSD1306_SetCursor(ssd1306_t* ssd1306, int position) {
	FontDef_t font;
	uint16_t x_position = 2;
	uint16_t y_position = 0;

	SSD1306_ResetCursor(ssd1306);

	if ((position >= 0) & (position <= 2)){
		y_position = position*Font_11x18.FontHeight;
		font = Font_11x18;
	} else if (position == 3){
		y_position = position*Font_11x18.FontHeight-1;
		font = Font_7x10;
	} else if (position == 4){
		x_position = SSD1306_WIDTH/2;
		y_position = ((position-1)*Font_11x18.FontHeight)-1;
		font = Font_7x10;
	} else {
		y_position = 0;
	}
	SSD1306_GotoXY(ssd1306, x_position, y_position);
	SSD1306_Putc(ssd1306, '>', &font, 1);
